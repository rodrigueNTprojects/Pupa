...
vote(rNominator, rNominee, rEndorser, endorser, pCase, onNomination, isAccepted) {
  ...
  checkProcessTransaction(txID: string) {
    this.http
    .post('http://localhost:3000/transaction/load', {from: txID})
    .subscribe(
      (resp) => {
        console.log('SUCCESS: ', resp.json());
    },
  ...
  loadProcessSteps(processID: string) {
    this.http
    .post('http://localhost:3000/processsteps/load', {from: processID})
    .subscribe(
      (resp) => {
        const resP = resp.json();
        resP.forEach(element => {
          //if ((element.name.indexOf(modelId) >= 0 || element.id.indexOf(modelId) >= 0) && this.processes.indexOf(element) < 0) {
            this.processesStepsList.push(element);
