...
collectControlFlowInfo = (proc, globalNodeMap, globalControlFlowInfo) => {
    ...
    if (nonBlockingBoundaryEvents.length > 0) {
        ...
        for (let eventId of nonBlockingBoundaryEvents) {
            ...
            if (mainPathNodeList.filter((nodeId) => localNodeList.indexOf(nodeId) >= 0).length > 0)
            ...
            let childControlFlowInfo = new definitions_1.ControlFlowInfo(event, localNodeList, localEdgeList, [eventId], []);
...
let extractParameters = (cad, nodeId, controlFlowInfo) => {
    ...
    if (controlFlowInfo != null) {
        ...
        for (let i = 0; i < toIterate.length; i += 2)
            inParameters.push(new definitions_1.ParameterInfo(toIterate[i], toIterate[i + 1]));
        toIterate = resMap.get('output');
        let parameters = new Map();
        parameters.set('input', inParameters);
        parameters.set('output', outParameters);
        for (let i = 0; i < toIterate.length; i += 2)
            outParameters.push(new definitions_1.ParameterInfo(toIterate[i], toIterate[i + 1]));
...
exports.parseModel = (modelInfo) => {
    return new Promise((resolve, reject) => {
        parseBpmn(modelInfo.bpmn)
            .then((definitions) => {
            modelInfo.solidity = "pragma solidity ^0.6.12;\n";
            ...
            let globalNodeMap = new Map(), globalNodeIndexMap = new Map(), globalEdgeIndexMap = new Map(), globalControlFlowInfo = [];
            ////////////////////////////////////////////////////////////
            ...
            let isTimerEventLead = nodeId => {
                let task = globalNodeMap.get(nodeId);                            
                if (task.incoming) {
                    let node = task.incoming[0].sourceRef;
                    return (is(node, "bpmn:IntermediateCatchEvent") && is(node.eventDefinitions[0], "bpmn:TimerEventDefinition"));
            ...
            for (let controlFlowInfo of globalControlFlowInfo) {
                if (!controlFlowInfo.isEmbedded) {
                    let multiinstanceActivities = [], callActivities = [], nonInterruptingEvents = [], catchingMessages = [], catchingTimers = [], nodeLeadbyTimer = [], multiIncomingInclGateway = [];
                    controlFlowInfo.nodeList
                        ...
                        if ((is(e, "bpmn:Task") || is(e, "bpmn:SubProcess")) && e.loopCharacteristics &&
                            ...
                            catchingMessages.push(e.id);
                        else if (is(e, "bpmn:IntermediateCatchEvent") && is(e.eventDefinitions[0], "bpmn:TimerEventDefinition"))
                            catchingTimers.push(e.id);
                        else if (is(e, "bpmn:InclusiveGateway") && e.incoming && e.incoming.length > 1)
                        multiIncomingInclGateway.push(e.id);
                    ...
                    // extract all node preceded by a timer
                    controlFlowInfo.nodeList.forEach(nodeId => {
                        if (isTimerEventLead(nodeId))
                            nodeLeadbyTimer.push(nodeId);
                    ...
                    controlFlowInfo.catchingTimers = catchingTimers;
                    controlFlowInfo.nodeLeadbyTimer = nodeLeadbyTimer;
                    controlFlowInfo.multiIncomingInclGateway = multiIncomingInclGateway
                    ...
                    let codeGenerationInfo = {
                        ...
                        catchingTimers: controlFlowInfo.catchingTimers,
                        nodeLeadbyTimer: controlFlowInfo.nodeLeadbyTimer,
                        multiIncomingInclGateway: controlFlowInfo.multiIncomingInclGateway,
                        ...
                        getWorkItemsGroupByParameters: (isInput) => {
                            ...
                            controlFlowInfo.nodeList.forEach(nodeId => {
                                let node = globalNodeMap.get(nodeId);
                                if (is(node, 'bpmn:UserTask') || is(node, 'bpmn:ReceiveTask') || catchingMessages.indexOf(nodeId) >= 0 || catchingTimers.indexOf(nodeId) >= 0) {
                                    let params = "";
                        ...
                        getWorkItemsGroupByParametersWithoutTimer: (isInput) => {
                            let name2Ids = new Map();
                            controlFlowInfo.nodeList.forEach(nodeId => {
                                let node = globalNodeMap.get(nodeId);
                        ...
                        subprocessNodeFullMarking: subprocId => {
                            ...
                            for (let i = bitarray.length - 1; i >= 0; i--)
                                result += bitarray[i] ? "1" : "0";
                            return result === '0b' ? new bignumber_js_1.default(0) : new bignumber_js_1.default(result).toFixed();
                        },
                        ...
                        getPreviousTimerNodeId: nodeId => {
                            let task = globalNodeMap.get(nodeId);                            
                            if (isTimerEventLead(nodeId)) {
                                let timerId = task.incoming[0].sourceRef.id;
                                return timerId;
                            }
                        ...
                        getTimeDuration: nodeId => {
                            let node = globalNodeMap.get(nodeId);
                            if (catchingTimers.indexOf(nodeId) >= 0) {
                                if (node.eventDefinitions[0].timeDuration && node.eventDefinitions[0].timeDuration.body.length > 0)
                                    return node.eventDefinitions[0].timeDuration.body;
                                else
                                    return "";
                            }                            
                            else
                        ...
                        listIncomingInclGateway: nodeId => {
                            let node = globalNodeMap.get(nodeId);
                            let res = [];
                            if(is(node, "bpmn:InclusiveGateway") && node.incoming && node.incoming.length > 1 ) {
                                node.incoming.forEach( incoming => {
                                    res.push([incoming.sourceRef.id, incoming]);
                                   });
                            }
                            return res;
                        },
                        getCondition: flowEdge => flowEdge.conditionExpression
                    ...
                    controlFlowInfo.nodeList.forEach(nodeId => {
                        let node = globalNodeMap.get(nodeId);
                        if (is(node, 'bpmn:UserTask') || is(node, 'bpmn:ReceiveTask') || catchingTimers.indexOf(nodeId) >= 0) {
                    ...
                    if (controlFlowInfo.catchingTimers.length > 0)
                        userTaskList = userTaskList.concat(controlFlowInfo.catchingTimers);
                    ...
                    let workListGenerationInfo = {
                        ...
                        getTimers: () => {
                            if (controlFlowInfo.catchingTimers.length > 0)
                                return controlFlowInfo.catchingTimers;
                            else
                                return [];
                        },
                        isTimerNode: (nodeId) => {
                            if (controlFlowInfo.catchingTimers.indexOf(nodeId) >= 0)
                                return true;
                            else
                                return false;
                        },
                        /* getPreviousTimerNodeId: nodeId => {
                            let task = globalNodeMap.get(nodeId);                            
                            if (isTimerEventLead(nodeId)) {
                                let timerId = task.incoming[0].sourceRef;
                                return timerId;
                            }
                            
                            return task;
                        }, */
                        getNodeIdLeadingTimer: nodeId => {
                            let task = globalNodeMap.get(nodeId);
                            let idNode =  nodeId;                          
                            if (isTimerNode(nodeId)) {
                                idNode = task.outgoing[0].targetRef.id;                                
                            }
                            return idNode;
