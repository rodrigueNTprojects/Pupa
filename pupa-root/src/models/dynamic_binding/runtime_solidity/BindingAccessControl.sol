...
abstract contract BindingPolicy {
    ...
    function isCaseCreator(uint roleIndex) virtual public pure returns(bool);
    function canNominate (uint rNominator, uint rNominee) virtual public pure returns(bool);
    function assertNConstraint (uint rNominator, uint rNominee, uint nomineeRoles) virtual public pure returns(bool);
    function requireNEndorsement (uint rNominator, uint rNominee) virtual public pure returns(bool);
    function assertNVote (uint rNominator, uint rNominee, uint rEndorser, uint endorsedBy, uint rejectedBy, bool isAccepted) virtual public pure returns(uint);
    
    function canRelease (uint rNominator, uint rNominee) virtual public pure returns(bool);
    function assertRConstraint (uint rNominator, uint rNominee, uint nomineeRoles) virtual public pure returns(bool);
    function requireREndorsement (uint rNominator, uint rNominee) virtual public pure returns(bool);
    function assertRVote (uint rNominator, uint rNominee, uint rEndorser, uint endorsedBy, uint rejectedBy, bool isAccepted) virtual public pure returns(uint);
}

abstract contract ControlFlow {
    function findParent() virtual public view returns(address);
    function getRoleFromTask(uint taskIndex, bytes32 processId) virtual public pure returns(uint);
    function bundleFor(address pCase) virtual external view returns(bytes32);
} 
...
contract BindingAccessControl {
    ...
    constructor(address _registry, address _policy, address _taskRoleAdr) public {
        ...
        require(address(_policy) != address(0));
        require(address(_taskRoleAdr) != address(0));
        policyAdr = _policy;
    ...
    function nominateCaseCreator (uint rNominee, address nominee, address pCase) public {
        ...
        require(BindingPolicy(policyAdr).isCaseCreator(rNominee));
    ...
    function nominate (uint rNominator, uint rNominee, address nominator, address nominee, address pCase) public {
        ...
        require(actorRoles[iNominator] & (uint(1) << rNominator) != 0 || (iNominator == 0 && rNominator == rNominee));
        require(BindingPolicy(policyAdr).canNominate(rNominator, rNominee));
        require(BindingPolicy(policyAdr).assertNConstraint(rNominator, rNominee, actorRoles[iNominee]));
